 ...
 * never leaves the host process or the browser.
 */
...
function readKey() {
  const home = homedir();
  try {
    const state = JSON.parse(readFileSync(join(home, ".dsh", ".ocg-state.json"), "utf8"));
    if (state && state.apiKey) return state.apiKey;
  } catch {}
  try {
